...
class MockAuthCategory extends AuthCategory {
  ...
  Future<AuthUser> getCurrentUser({
    ...
    return const AuthUser(userId: 'mock_current_user_id', username: 'mock_username');
  }
...
class MockDataStoreCategory extends DataStoreCategory {
  ...
  MockDataStoreCategory({required this.usersToReturn, this.currentUserToReturn});
  ...
  Future<List<T>> query<T extends Model>(
    ...
    if (modelType == User.classType) {
      if (where != null) {
        ...
        if (currentUserToReturn != null && where.toString().contains(currentUserToReturn!.id)) {
  ...
  @override
  Stream<SubscriptionEvent<T>> observe<T extends Model>(ModelType<T> modelType, {QueryPredicate? where}) {
...
void main() {
  ...
  setUp(() {
    ...
    final mockDataStore = MockDataStoreCategory(usersToReturn: allMockUsers, currentUserToReturn: mockCurrentUser);
  ...
  testWidgets('CommunityScreen: Tap on user navigates to UserDetailScreen', (WidgetTester tester) async {
    ...
    await tester.pumpAndSettle(const Duration(seconds: 2)); // Allow time for any async operations
    ...
    expect(find.text('User Details'), findsOneWidget); // AppBar title of UserDetailScreen
    expect(find.text('Global User One'), findsOneWidget); // Username on UserDetailScreen
    ...
    expect(navigator.userGestureInProgress, isFalse); // Ensure no gesture is active
  ...
  testWidgets('CommunityScreen: Long press shows EnlargedUserPreview overlay', (WidgetTester tester) async {
    await tester.pumpWidget(createTestableWidget(CommunityScreen()));
    ...
    await tester.pumpAndSettle(); // Allow overlay to build and animations (if any)

    // Verify EnlargedUserPreview is present
    expect(find.byType(EnlargedUserPreview), findsOneWidget);
    expect(find.text('Global User One'), findsNWidgets(2)); // One in list, one in preview

    // Verify BackdropFilter is present
    expect(find.byType(BackdropFilter), findsOneWidget);
  });

  testWidgets('CommunityScreen: Dismiss EnlargedUserPreview overlay on tap', (WidgetTester tester) async {
    await tester.pumpWidget(createTestableWidget(CommunityScreen()));
    await tester.pumpAndSettle(const Duration(seconds: 2));
    ...
    final stackFinder = find.ancestor(of: backdropFilter, matching: find.byType(Stack));
