...
import 'package:sighttrack/util.dart'; // Assuming path for Util
...
class MockUserAmplify extends User {
  ...
  // If your User model is generated by Amplify and uses a copyWith method,
  // you might need to override it or ensure your mock is compatible.
  // For this test, we'll assume direct construction is sufficient.
}
...
void main() {
  ...
  setUp(() {
    ...
    // Let's proceed assuming the CommunityScreen has been designed/refactored for testability
    ...
    // A common pattern is to provide a way to set the Amplify instance for testing.
    // e.g. `Amplify = mockAmplify;` (if Amplify was a settable static field)
    ...
    //
    // Given the limitations, we'll write the tests as if the mocks *can* be injected/used.
    // The actual execution in a real project might require adjusting how Amplify is used or mocked.
  });
  ...
  testWidgets('CommunityScreen: Dismiss EnlargedUserPreview overlay on tap', (WidgetTester tester) async {
    ...
    // Tap on the dismissal GestureDetector (usually the first one in the Stack, covering the screen)
    // This gesture detector is part of the overlay stack defined in CommunityScreen._showEnlargedPreview
    // It's a full-screen GestureDetector, so tapping anywhere on the overlay (outside the preview content if it doesn't consume taps)
    // This finds the BackdropFilter, then goes up to its parent Stack, then finds the first GestureDetector in that Stack.
    final backdropFilter = find.byType(BackdropFilter);
    expect(backdropFilter, findsOneWidget);
    
    // The dismissal GestureDetector is the one that makes the BackdropFilter its child's sibling, typically
    // by being the first child of a Stack that also contains the BackdropFilter and the Positioned preview.
    // Let's try to find it by looking for a GestureDetector that is a direct child of a Stack
    // and is likely the one for dismissal (often the first one).
    // This is a bit fragile. A key on the dismissal GestureDetector would be more robust.
